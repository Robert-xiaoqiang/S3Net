...
class conv_2nV1_rgbd_dev_decay(nn.Module):
    def __init__(self, in_hc=64, in_lc=256, out_c=64, main=0):
        super().__init__()
        self.main = main
        mid_c = min(in_hc, in_lc)
        ...
        if self.main == 0:
            ...
            raise NotImplementedError
        self.out_conv = nn.Conv2d(mid_c, out_c, 3, 1, 1)
...
class conv_3nV1_rgbd_dev_decay(nn.Module):
    def __init__(self, in_hc=64, in_mc=256, in_lc=512, out_c=64):
        super().__init__()
        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')
        self.downsample = nn.AvgPool2d((2, 2), stride=2)
...
class AIMRGBD_decay(nn.Module):
    def __init__(self, iC_list, oC_list):
        super().__init__()
        ic0, ic1, ic2, ic3, ic4 = iC_list
        oc0, oc1, oc2, oc3, oc4 = oC_list
        self.conv0 = conv_2nV1_rgbd_dev_decay(in_hc=ic0, in_lc=ic1, out_c=oc0, main=0)
        self.conv1 = conv_3nV1_rgbd_dev_decay(in_hc=ic0, in_mc=ic1, in_lc=ic2, out_c=oc1)
        self.conv2 = conv_3nV1_rgbd_dev_decay(in_hc=ic1, in_mc=ic2, in_lc=ic3, out_c=oc2)
        self.conv3 = conv_3nV1_rgbd_dev_decay(in_hc=ic2, in_mc=ic3, in_lc=ic4, out_c=oc3)
        self.conv4 = conv_2nV1_rgbd_dev_decay(in_hc=ic3, in_lc=ic4, out_c=oc4, main=1)
